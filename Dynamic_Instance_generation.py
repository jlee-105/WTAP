...
def input_generation(NUM_WEAPON, NUM_TARGET, value, prob, TW, max_time, batch_size):
    ...
    if value is None:
        ...
        for batch in range(TRAIN_BATCH):
            ...
            for target_index in range(NUM_TARGETS):
                # target related parameter
                t_value[target_index] = random.randint(MIN_TARGET_VALUE, MAX_TARGET_VALUE)
                t_value_scaled[target_index] = t_value.clone()[target_index].unsqueeze(0)
                time_target_emerging[target_index] = random.randint(0, 3)
                time_target_remove[target_index] = MAX_TIME_WINDOW[0]
            ...
            for w in range(NUM_WEAPON):
                for t in range(NUM_TARGETS):
                    ...
                    remaining_ammunition = torch.tensor([AMM[w]]).to(DEVICE)
                    # remaining time
                    remaining_time = torch.tensor([max_time]).to(DEVICE)
                    ...
                    n_of_fire_on_target = number_of_fired_on_target[t]
        ...
        weapon_to_target_prob = weapon_to_target_prob.reshape(TRAIN_BATCH, NUM_WEAPON, NUM_TARGET)
    ...
    # TO DO THIS PART---------------------------------------------
        ...
        for batch in range(VAL_BATCH):
            ...
            for target_index in range(NUM_TARGETS):
                t_value[target_index] = value[target_index]
                t_value_scaled[target_index] = t_value[target_index]
                time_target_emerging[target_index] = TW[target_index][0]
                time_target_remove[target_index] = TW[target_index][1]
            ...
            time_target_start_NO_ACTION = torch.tensor([0]).to(DEVICE)
            Number_of_fired_NO_ACTION = torch.tensor([0]).to(DEVICE)
            left_over_time_NO_ACTION = torch.tensor([max_time]).to(DEVICE)
            weapon_available_NO_ACTION = torch.tensor([0.0]).to(DEVICE)
            wait_to_ready_NO_ACTION = torch.tensor([-1.0]).to(DEVICE)
            target_value_NO_ACTION = torch.tensor([-1.0]).to(DEVICE)
            probability_NO_ACTION = torch.tensor([0.0]).to(DEVICE)
            active_time_NO_ACTION = torch.tensor([0.0]).to(DEVICE)
